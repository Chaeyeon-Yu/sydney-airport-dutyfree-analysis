...
-- Joins DIM_FLIGHTS on FLIGHT_NO + SALE_DATE for accurate daily matching.
------------------------------------------------------------
...
LEFT JOIN SYDNEY_DF_DB.SILVER.DIM_FLIGHTS fl
    ...
    AND f.SALE_DATE = CAST(fl.DEPARTURE_TIMESTAMP AS DATE)
...
category_pairs AS (
    ...
    LEFT JOIN SYDNEY_DF_DB.SILVER.DIM_FLIGHTS fl
        ON a.FLIGHT_NO = fl.FLIGHT_NO
        AND a.SALE_DATE = CAST(fl.DEPARTURE_TIMESTAMP AS DATE)
